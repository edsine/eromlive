                                            ...
                                            <form method="POST" action="{{ route('payment.remita') }}"
                                                ...
                                                <div class="row">
                                                    <div class="col-6">
                                                        ...
                                                        if ($service_app) {
    $app_form_fee_type = \App\Models\InspectionFee::where('branch_id', $user->branch->id)->where("service_id", $service_app->service_id)->where("processing_type_id", $service_app->service_type_id)->first();
    if ($app_form_fee_type && $app_form_fee_type->processing_type_id != 0) {
        $app_form_fee = $app_form_fee_type;
    } else {
        $app_form_fee = \App\Models\InspectionFee::where('branch_id', $user->branch->id)->where("service_id", $service_app->service_id)->first();
    }
} else {
